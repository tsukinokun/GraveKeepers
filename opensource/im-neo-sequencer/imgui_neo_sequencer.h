...
namespace ImGui {
    ...
    IMGUI_API bool BeginNeoGroup(const char* label, bool* open = nullptr, bool enable_group_select = true);
    ...
    IMGUI_API int GetNeoKeySelected();
    ...
    template< class T >
    bool BeginNeoTimeline(const char* label, std::vector<uint32_t>& keyframes, std::vector<T>& valueframes, bool* open = nullptr, ImGuiNeoTimelineFlags flags = ImGuiNeoTimelineFlags_None);
}
