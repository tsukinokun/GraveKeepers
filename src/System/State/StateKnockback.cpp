...
void StateKnockback::Init()
{
    ...
    if(auto model = owner->GetComponent<ComponentModel>()) {
        model->PlayAnimationNoSame("knockback", false);
    }
    //---------------------------------------------------------------------------
    // 更新処理の登録
    //---------------------------------------------------------------------------
...
void StateKnockback::GUI()
{
    ...
    {
        ImGui::Separator();
        if(ImGui::TreeNode(u8"StateKnockback")) {
...
CEREAL_REGISTER_TYPE(StateKnockback)
CEREAL_REGISTER_POLYMORPHIC_RELATION(Component, StateKnockback)
